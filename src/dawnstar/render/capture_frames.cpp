...
void noCanvasThread(GameCanvas *) {}
...
void checkWholeSurfaceRedrawn(Game *game, const char *what) {
    const uint32_t kSentinel = 0xFFFF00FFu;
    {
        std::lock_guard<std::mutex> guard(g_renderer.frameMutex());
        for (uint32_t &pixel : g_screen.pixels) pixel = kSentinel;
    }
    if (game->gameCanvas_ != nullptr) game->gameCanvas_->repaint();
    settle(900);
    ...
    {
        std::lock_guard<std::mutex> guard(g_renderer.frameMutex());
        for (int32_t y = 0; y < g_screen.height; ++y) {
            const uint32_t *row = g_screen.row(y);
            for (int32_t x = 0; x < g_screen.width; ++x) {
                if (row[x] == kSentinel) {
                    ++stale;
    ...
    if (stale != 0) {
        std::printf("  FAIL: %s left %zu pixels unredrawn, first at row %d\n", what,
                    stale, (int)firstRow);
        ++g_failures;
    } else {
        std::printf("  %s redraws every pixel\n", what);
    }
    std::fflush(stdout);
}
...
void useOptionsRow(Game *game, int32_t row) {
    press(game, 55, 700);
    for (int n1 = 0; n1 < 30; ++n1) {
        int32_t cur = g_game->currentUI_ != nullptr ? g_game->currentUI_->selectedIndex() : -1;
        if (cur < 0 || cur == row) break;
        press(game, cur < row ? KEY_DOWN : KEY_UP, 180);
    }
    press(game, KEY_SOFT_RIGHT, 800);
    for (int n2 = 0; n2 < 5 && g_game->currentUI_ != nullptr; ++n2) {
        press(game, KEY_SOFT_LEFT, 400);
    }
}

void checkShortcut(Game *game, const char *what, int32_t key, int32_t expectScreen) {
    if (!shortcuts::onKey(key)) {
        std::printf("  FAIL: %s shortcut refused (not in game?)\n", what);
        ++g_failures;
        return;
    }
    bool saw = false;
    for (int n1 = 0; n1 < 80 && (shortcuts::pending() || !saw); ++n1) {
        shortcuts::tick();
        if (g_game->currentUI_ != nullptr && g_game->currentUI_->screenId_ == expectScreen) saw = true;
        tickOnce();
        if (g_game->currentUI_ != nullptr && g_game->currentUI_->screenId_ == expectScreen) saw = true;
    }
    settle(400);
    if (g_game->currentUI_ != nullptr && g_game->currentUI_->screenId_ == expectScreen) saw = true;
    int32_t got = saw ? expectScreen : (g_game->currentUI_ != nullptr ? g_game->currentUI_->screenId_ : -1);
    if (got != expectScreen) {
        std::printf("  FAIL: %s shortcut landed on screen %d, expected %d\n", what,
                    (int)got, (int)expectScreen);
        ++g_failures;
    } else {
        std::printf("  %-22s -> screen %d\n", what, (int)got);
    }
    std::fflush(stdout);
    for (int n2 = 0; n2 < 4 && g_game->currentUI_ != nullptr; ++n2) {
        Canvas *canvas = currentCanvas(game);
        canvas->keyPressed(KEY_SOFT_LEFT);
        canvas->keyReleased(KEY_SOFT_LEFT);
        settle(300);
    }
}
...
bool completeNameForm(const std::string &name, int timeoutMs) {
    ...
    while (g_game->platformContext_->nowMillis() < deadline) {
        ...
        g_game->platformContext_->sleepFor(50);
    }
    if (form == nullptr) {
...
int main(int argc, char **argv) {
    ...
    Game *game = nullptr;
    dawnstar_widescreen::Context widescreenContext(&g_renderer, &g_screen, &game);
    dawnstar_raycast::Context raycastContext(&g_renderer, &g_screen, &game);
    dawnstar_init_statics(&captureContext);
    ...
    if (!expectScreen("class list", 3, 8000)) return 1;
    ...
    press(game, KEY_SOFT_RIGHT, 500);
    if (!expectScreen("intro 1", 101, 8000)) return 1;
    snapshot(outDir, "06_intro");

    press(game, KEY_SOFT_RIGHT, 500);
    if (!expectScreen("intro 2", 102, 8000)) return 1;
    ...
    settle(800);
    snapshot(outDir, "16_wide_dungeon");
    press(game, -4, 900);
    ...
    for (int attempt = 0; attempt < 4; ++attempt) {
        int8_t beforeX = game->character_->gridX_;
        int8_t beforeY = game->character_->gridY_;
        press(game, 50, 900);
        if (game->character_->gridX_ != beforeX || game->character_->gridY_ != beforeY) break;
        press(game, -4, 700);
    }
    snapshot(outDir, "24_3d_step1");
    {
        int8_t beforeX = game->character_->gridX_;
        int8_t beforeY = game->character_->gridY_;
        press(game, 50, 900);
        if (game->character_->gridX_ == beforeX && game->character_->gridY_ == beforeY) {
            std::printf("  FAIL: could not take a second step to test the flip\n");
            ++g_failures;
        }
    }
    snapshot(outDir, "25_3d_step2");
    checkWholeSurfaceRedrawn(game, "3D view, camp");

    {
        Dungeon *ice = game->dungeons_.as<Dungeon>(1);
        int32_t openX = -1;
        int32_t openY = -1;
        for (int32_t x = 1; x < ice->width_ - 1 && openX < 0; ++x) {
            for (int32_t y = 1; y < ice->height_ - 1; ++y) {
                if (!GameUtil::hasFlag((int8_t)1, ice->tiles_[x][y])) {
                    openX = x;
                    openY = y;
                    break;
                }
        ...
        if (openX < 0) {
            std::printf("  FAIL: found no open cell in the ice dungeon\n");
            ++g_failures;
        } else {
            game->character_->dungeonId_ = game->character_->nextDungeon_ = 2;
            game->character_->gridX_ = game->character_->nextX_ = (int8_t)openX;
            game->character_->gridY_ = game->character_->nextY_ = (int8_t)openY;
            game->character_->refreshSurroundings();
            game->gameCanvas_->mapDirty_ = true;
            settle(700);
            snapshot(outDir, "26_3d_ice");
            checkWholeSurfaceRedrawn(game, "3D view, short ice floor");
        }
    }

    {
        ...
        UIWidget *wrapped = new UIWidget(game, uistate::LAYOUT_FORM_1, 900);
        SharedArray<std::string> rows(3);
        rows[0] = std::string("An entry far too wide to fit across the screen in one line");
        rows[1] = std::string("Short one");
        rows[2] = std::string("Short two");
        wrapped->setupForm(std::string("Wrapped"), std::string("Body text"), rows);
        UIWidget *savedCurrent = g_game->currentUI_;
        g_game->currentUI_ = wrapped;
        g_game->uiCanvas_->widget_ = wrapped;

        wrapped->setSelectedIndex(0);
        wrapped->requestRepaint();
        wrapped->flushRepaints();
        snapshot(outDir, "27_wrapped_row");

        wrapped->setSelectedIndex(1);
        wrapped->requestRepaint();
        wrapped->flushRepaints();
        snapshot(outDir, "28_after_wrapped_row");
    ...
    raycastContext.setEnabled(false);
    widescreenContext.setSceneOwnedExternally(false);
    widescreenContext.setEnabled(false);
    dawnstar_shortcuts::install(&game);
    {
        const shortcuts::Binding bindings[] = {
            {1001, 1, "inventory"}, {1002, 3, "skills"}, {1003, 6, "quicksave"},
        };
        shortcuts::install(dawnstar_shortcuts::host(), bindings, 3);
    }
    settle(400);
    checkShortcut(game, "inventory (I)", 1001, 33);
    checkShortcut(game, "skills (K)", 1002, 35);

    useOptionsRow(game, 3);
    checkShortcut(game, "inventory after stale row", 1001, 33);

    checkShortcut(game, "quicksave (F6)", 1003, 303);
