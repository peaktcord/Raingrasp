...
int main(int argc, char **argv) {
    ...
    SaveRecordFiles::setRoot("saves/rms-dungeondump");
    ...
    for (int32_t n1 = 0; n1 < count; ++n1) {
        ...
        int32_t monsters = (int32_t)game->worldState_.monsters.at((std::size_t)n1).size();
        int32_t chests = (int32_t)game->worldState_.chests.at((std::size_t)n1).size();
        row(std::string("dungeon\t") + i2s(n1) + "\t" + dungeon->name() + "\t" +
