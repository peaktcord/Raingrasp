...
void runHelperJobInline(Game *game, int32_t job) { game->runHelperJob(job); }
void noCanvasThread(GameCanvas *) {}
void noSplashThread(UIWidget *) {}

class Host : public host::GameHost {
public:
    const char *id() const override { return "dawnstar"; }
    const char *title() const override { return "The Elder Scrolls Travels: Dawnstar"; }
    const char *defaultSaveDir() const override { return "saves/rms"; }
    ...
    void boot(platform::PlatformContext *context, render::Context *renderer,
        ...
        context_ = context;
        if (renderer != nullptr) {
            widescreen_ = std::make_unique<dawnstar_widescreen::Context>(renderer, screen, &game_);
            raycast_ = std::make_unique<dawnstar_raycast::Context>(renderer, screen, &game_);
        }
        dawnstar_shortcuts::install(&game_);
        dawnstar_init_statics(context);

        owned_ = std::make_unique<Game>(profile(), context);
        game_ = owned_.get();
        game_->setExecutionHooks(runHelperJobInline, noCanvasThread, noSplashThread);
    ...
    const shortcuts::Host *shortcutHost() override { return dawnstar_shortcuts::host(); }
    ...
    std::string bootSummary() override {
        ...
        out += line;
        std::snprintf(line, sizeof(line), "help titles: %d\n", (int)game_->helpTitles_.length());
        out += line;
        std::snprintf(line, sizeof(line), "dungeons: %d\n", (int)game_->dungeons_.size());
        out += line;
        int totalMonsters = 0, totalChests = 0;
        for (int n1 = 0; n1 < 37; ++n1) {
            totalMonsters += (int)game_->worldState_.monsters.at((std::size_t)n1).size();
            totalChests += (int)game_->worldState_.chests.at((std::size_t)n1).size();
        }
        std::snprintf(line, sizeof(line), "monsters placed: %d, chests placed: %d\n", totalMonsters,
                      totalChests);
        out += line;
        std::snprintf(line, sizeof(line), "dungeon 2 name: %s  size %dx%d\n",
                      game_->dungeons_.as<Dungeon>(1)->name().c_str(), (int)game_->dungeons_[1]->width_,
                      (int)game_->dungeons_[1]->height_);
        out += line;
        for (int di = 1; di < 37; di += 12) {
            ...
            for (int x = 0; x < dg->width_; ++x) {
                for (int y = 0; y < dg->height_; ++y) {
                    h = (h ^ (unsigned char)dg->tiles_[x][y]) * 16777619u;
                }
            }
            std::snprintf(line, sizeof(line), "dungeon %d layout digest: %08lx (monsters %d chests %d)\n",
                          di + 1, h & 0xFFFFFFFFul,
                          (int)game_->worldState_.monsters.at((std::size_t)di).size(),
                          (int)game_->worldState_.chests.at((std::size_t)di).size());
            out += line;
        }
        out += "SMOKE OK\n";
        return out;
...
private:
    ...
    std::unique_ptr<dawnstar_widescreen::Context> widescreen_;
    std::unique_ptr<dawnstar_raycast::Context> raycast_;
};
